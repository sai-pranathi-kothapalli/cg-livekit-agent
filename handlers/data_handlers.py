...
def _handle_code_observation(data: rtc.DataPacket, session, log) -> None:
    ...
    f"2. Do NOT say the code is wrong or incomplete based on visual inspection alone.\n"
...
def _handle_code_submission(data: rtc.DataPacket, session, log) -> None:
    ...
    try:
        ...
        evaluation_context = (
            ...
            f"REQUIRED — EXPLAIN EXECUTION ERRORS:\n"
            f"- If `execution_output` contains a syntax error or runtime error, YOU MUST EXPLICITLY MENTION IT.\n"
            f"- Instead of saying 'Your logic is wrong', say something like 'It seems there is a syntax error in your submission' or 'The code failed to run with the following error...'.\n\n"
            ...
            f"2. NEVER read the entire code aloud or say 'I see you wrote...'.\n"
            f"3. If fast (<60s) for medium problem, internally note suspicious speed.\n"
            f"4. Tell them clearly if it works or where it fails based on the Execution Output.\n"
